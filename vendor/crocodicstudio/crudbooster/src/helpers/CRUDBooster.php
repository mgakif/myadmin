...
class CRUDBooster  {		
		...
		public static function sendEmail($config=[]) {   
		    ...
		    if($config['send_at']!=NULL) {
				...
				$a['email_from_email']  = "mgakif@yandex.com";
		    ...
		    Mail::send("crudbooster::emails.blank",['content'=>$html],function($message) use ($to,$subject,$template,$attachments) {
		        ...
		        if($template->from_email) {
		        	...
		        	$message->from('mgakif@yandex.com',$from_name);
		        }
		  ...
		  /*  
		Mail::send('crudbooster::emails.bos', ['content'=>$html], function($message) {
        $message->to('mgakif@gmail.com', 'Mehmet Akif Garip')->subject('Yeni İletişim Bilgisi');
        });*/
		}
